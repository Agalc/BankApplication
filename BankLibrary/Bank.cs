...
{
    public enum AccountType//type of account
    {
        Ordinary,
        Deposit
    }
    ...
    public class Bank<T> where T : Account
    {
        ...
        public void Put(decimal val, int id)//add 
        {
        ...
        public void Withdraw(decimal val, int id)
        {
            T account = FindAccount(id);
            if (account == null)
                throw new Exception("Счет не найден");
        ...
        public void Close(int id)//close account
        {
            int index;
            ...
            {
                //reduce account array by deleting closed account
                T[] tempAccounts = new T[accounts.Length - 1];
        ...
        public T FindAccount(int id)//search account by id
        {
            for (int i = 0; i < accounts.Length; i++)
            {
                if (accounts[i].Id == id)
        ...
        public T FindAccount(int id, out int index)// overrided version
        {
            for (int i = 0; i < accounts.Length; i++)
            {
                if (accounts[i].Id == id)
